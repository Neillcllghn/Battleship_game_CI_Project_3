...
# credit to Knowledge Mavens - How to Code Battleship in Python
...
def validate_username(values):
    ...
    if len(values) < 3 or len(values) > 6:
        print("Username must be more than 3 and less than 6 characters long, "
              f"you provided {len(values)}.")
        return False
    elif any(char.isnumeric() for char in values):
        print(f"the username {values} cannot be used, "
              "please don't use numbers.")
        return False
...
def no_of_ships():
    ...
    while not ships.isdigit() or int(ships) < 1 or int(ships) >= 11:
        print(f'You selected invaild {ships} number of ships, '
              'please try again')
        ships = input('How many ships do you want to sink? Between 1-10 :\n')
    return int(ships)
...
def no_of_turns():
    ...
    while not turns.isdigit() or int(turns) < 1 or int(turns) >= 21:
        print(f'You selected invaild {turns} number of turns, '
              'please try again')
        turns = input('How many turns do you want? Between 1-20 :\n')
    return int(turns)
